...
import { CropFactorCalculatorCard } from "@/components/crop-factor-calculator-card";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "CropFactorCalculator" });
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_URL}/${locale}/tools/crop-factor-calculator`,
      languages: languageAlternates("/tools/crop-factor-calculator"),
    },
    openGraph: {
      type: "website",
...
export default async function CropFactorCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("CropFactorCalculator");
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-10">
      ...
      <CropFactorCalculatorCard />

      <AdZone
        id="crop-factor-calculator-mid"
        label="Ad"
        size="300×250"
      ...
      {process.env.NODE_ENV === "development" ? (
        <ToolImageDevPanel
          slug="crop-factor-calculator"
          left={getToolImages("crop-factor-calculator").left}
          right={getToolImages("crop-factor-calculator").right}
          defaultQuery="camera lens sensor comparison mirrorless dslr"
        />
      ) : null}
    </div>
  );
