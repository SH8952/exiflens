...
import { AstroCalculatorCard } from "@/components/astro-calculator-card";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "AstroCalculator" });
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_URL}/${locale}/tools/astrophotography-calculator`,
      languages: languageAlternates("/tools/astrophotography-calculator"),
    },
    openGraph: {
      type: "website",
      ...
      title,
      description,
      url: `${SITE_URL}/${locale}/tools/astrophotography-calculator`,
    },
  };
}

export default async function AstrophotographyCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("AstroCalculator");
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-10">
      ...
      <ToolExampleImages
        slug="astrophotography-calculator"
      ...
      <AdZone
        id="astrophotography-calculator-mid"
        label="Ad"
        size="300×250"
      ...
      {process.env.NODE_ENV === "development" ? (
        <ToolImageDevPanel
          slug="astrophotography-calculator"
          left={getToolImages("astrophotography-calculator").left}
          right={getToolImages("astrophotography-calculator").right}
          defaultQuery="milky way night sky stars long exposure landscape"
        />
      ) : null}
    </div>
  );
