...
import { TimelapseCalculatorCard } from "@/components/timelapse-calculator-card";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "TimelapseCalculator" });
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_URL}/${locale}/tools/timelapse-calculator`,
      languages: languageAlternates("/tools/timelapse-calculator"),
    },
    openGraph: {
      type: "website",
      ...
      title,
      description,
      url: `${SITE_URL}/${locale}/tools/timelapse-calculator`,
    },
  };
}

export default async function TimelapseCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("TimelapseCalculator");
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-10">
      ...
      {process.env.NODE_ENV === "development" ? (
        <ToolImageDevPanel
          slug="timelapse-calculator"
          left={getToolImages("timelapse-calculator").left}
          right={getToolImages("timelapse-calculator").right}
          defaultQuery="clouds moving timelapse long exposure sky"
        />
      ) : null}
    </div>
  );
