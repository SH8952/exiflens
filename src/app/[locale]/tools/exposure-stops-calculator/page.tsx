...
import { ExposureCalculatorCard } from "@/components/exposure-calculator-card";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "ExposureCalculator" });
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_URL}/${locale}/tools/exposure-stops-calculator`,
      languages: languageAlternates("/tools/exposure-stops-calculator"),
    },
    openGraph: {
      type: "website",
      ...
      title,
      description,
      url: `${SITE_URL}/${locale}/tools/exposure-stops-calculator`,
    },
  };
}

export default async function ExposureStopsCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("ExposureCalculator");
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-10">
      ...
      <ToolExampleImages
        slug="exposure-stops-calculator"
        leftAlt={t("exampleLeftAlt")}
        rightAlt={t("exampleRightAlt")}
      />

      <ExposureCalculatorCard />

      <AdZone
        id="exposure-stops-calculator-mid"
        label="Ad"
        size="300×250"
      ...
      {process.env.NODE_ENV === "development" ? (
        <ToolImageDevPanel
          slug="exposure-stops-calculator"
          left={getToolImages("exposure-stops-calculator").left}
          right={getToolImages("exposure-stops-calculator").right}
          defaultQuery="fast shutter speed freeze motion splash photography"
        />
      ) : null}
    </div>
  );
