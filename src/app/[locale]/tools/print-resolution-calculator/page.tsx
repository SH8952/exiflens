...
import { PrintResolutionCalculatorCard } from "@/components/print-resolution-calculator-card";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "PrintResolutionCalculator" });
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_URL}/${locale}/tools/print-resolution-calculator`,
      languages: languageAlternates("/tools/print-resolution-calculator"),
    },
    openGraph: {
      type: "website",
      ...
      title,
      description,
      url: `${SITE_URL}/${locale}/tools/print-resolution-calculator`,
    },
  };
}

export default async function PrintResolutionCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("PrintResolutionCalculator");
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-10">
      ...
      <PrintResolutionCalculatorCard />

      <AdZone
        id="print-resolution-calculator-mid"
        label="Ad"
        size="300×250"
      ...
      {process.env.NODE_ENV === "development" ? (
        <ToolImageDevPanel
          slug="print-resolution-calculator"
          left={getToolImages("print-resolution-calculator").left}
          right={getToolImages("print-resolution-calculator").right}
          defaultQuery="large photo print poster printing lab"
        />
      ) : null}
    </div>
  );
