...
import { StorageCalculatorCard } from "@/components/storage-calculator-card";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "StorageCalculator" });
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_URL}/${locale}/tools/storage-calculator`,
      languages: languageAlternates("/tools/storage-calculator"),
    },
    openGraph: {
      type: "website",
      ...
      title,
      description,
      url: `${SITE_URL}/${locale}/tools/storage-calculator`,
    },
  };
}
...
export default async function StorageCalculatorPage({
  ...
  const t = await getTranslations("StorageCalculator");
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-10">
      ...
      <StorageCalculatorCard />

      <AdZone
        id="storage-calculator-mid"
        label="Ad"
        size="300×250"
      ...
      {process.env.NODE_ENV === "development" ? (
        <ToolImageDevPanel
          slug="storage-calculator"
          left={getToolImages("storage-calculator").left}
          right={getToolImages("storage-calculator").right}
          defaultQuery="memory card sd card camera storage backup drive"
        />
      ) : null}
    </div>
  );
