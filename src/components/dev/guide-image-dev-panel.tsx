 ...
 * 뜨는 플로팅 도구. 검색어로 Unsplash 후보 이미지 여러 장을 받아온 뒤,
 * 그중 하나를 골라 적용하면 해당 글의 대표 이미지(webp)와 4개 언어
 * mdx frontmatter(image/imageCredit/imageCreditUrl)가 즉시 갱신된다.
 *
 * git add/commit/push는 하지 않으므로, 적용 후에는 기존 방식대로 push
 * 스크립트로 커밋해야 한다. 렌더링 여부(NODE_ENV 체크)는 이 컴포넌트를
 * 사용하는 서버 컴포넌트(guides/[slug]/page.tsx) 쪽에서 처리한다 - 이
 * 컴포넌트 자체는 항상 렌더링을 시도하므로 프로덕션 코드에 절대로 직접
 * 렌더링하지 않는다.
 *
        ...
        <div className="flex max-h-[80vh] flex-col gap-3 overflow-y-auto rounded-lg border border-amber-500 bg-background p-4 shadow-xl">
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder="영문 검색어 (예: tripod photography)"
              className="text-xs"
            />
