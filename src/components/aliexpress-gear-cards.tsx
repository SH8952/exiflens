...
// src/app/api/aliexpress/search/route.ts가 이를 읽어 실제 방문자 화면에도
// 즉시 반영한다. `process.env.NODE_ENV`는 Next.js가 클라이언트 번들에도
// 빌드 시점에 리터럴로 치환하므로, 프로덕션 빌드에서는 이 블록 전체가 데드
// 코드로 제거된다(src/app/[locale]/layout.tsx의 DevServerWatch 조건부
// 렌더링과 동일한 방식).
...
export function AliexpressGearCards() {
  ...
  const handleAdsBlock = React.useCallback(async () => {
    ...
    try {
      ...
      // 낙관적 업데이트: 방금 차단한 상품은 새로고침 없이도 화면에서 바로 제거
      setState((prev) =>
        prev?.status === "ok"
