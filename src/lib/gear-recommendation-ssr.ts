 ...
 * 서버 HTML에 실제 상품을 심어둔다 (ND 필터 계산기의 filterId와 무관하게
 * 공통 브랜드 키워드 하나만 쓴다 — CoupangGearCards의 filterId는 어차피
 * /api/coupang/search가 읽지 않아 검색 결과에 영향을 주지 않음). 이 조회는
 * searchCoupangProducts / searchAliexpressProducts에 이미 걸려있는 6시간 캐싱
 * (next: {revalidate: 21600})을 그대로 타므로 요청마다 실제 API를 호출하지
 * 않는다 — 쿠팝 시간당 10회 제한에 영향 없음. 실패(자격 증명 미설정, API
 * 일시 오류 등) 시에는 null을 반환해 기존 동작(클라이언트 위젯이 최종적으로
 * 콘텐츠를 채우는 것)으로 조용히 폴백한다.
 */

// src/app/api/coupang/search/route.ts의 ROW1_KEYWORDS[0]과 동일하게 유지할 것
const SSR_COUPANG_KEYWORD = "에이치앤와이 nd";
// src/app/api/aliexpress/search/route.ts의 ROW1_KEYWORDS[0]과 동일하게 유지할 것
const SSR_ALIEXPRESS_KEYWORD = "nisi nd";
