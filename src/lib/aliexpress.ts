 ...
 * were tried on 2026-09-06 and all failed with a live "IncompleteSignature"
 * error). The implementation below is instead copied as closely as
 * possible from a *confirmed-working* call to this exact API — the same
 * app_key/app_secret pair, used against this same
 * aliexpress.affiliate.product.query method, already serving live product
 * data in the user's separate naver-blog-automation project
 * (searchAliexpressProduct() in that project's main.js). Do not "improve"
 * this signature/param shape without re-verifying against a real request —
 * every deviation from this exact shape (extra params, split
 * query/body, a different timestamp format) has broken it in practice.
 * If AliExpress ever changes this, the symptom will be a JSON body with an
 * `error_response` key — see parseProductQueryResponse below, which
 * surfaces that case as an AliexpressApiError with the raw error message.
 */
...
export async function searchAliexpressProducts(
  ...
  // fields, no partner_id. tracking_id is only added when present, exactly
  // like the reference, since that project independently confirmed that an
  // always-present-but-empty tracking_id silently breaks ad insertion.
